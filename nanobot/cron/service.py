...
def _compute_next_run(schedule: CronSchedule, now_ms: int) -> int | None:
    ...
    if schedule.kind == "cron" and schedule.expr:
        if not _croniter_available():
            logger.warning("croniter is not installed; cron schedules are disabled")
            return None
        try:
...
class CronService:
    ...
    def _recompute_next_runs(self) -> None:
        ...
        if not self._store:
            return
        now = _now_ms()
        cron_ok = _croniter_available()
        for job in self._store.jobs:
            if job.enabled:
                if job.schedule.kind == "cron" and not cron_ok:
                    job.state.next_run_at_ms = None
                    job.state.last_status = "error"
                    job.state.last_error = "croniter not installed; cron schedules disabled"
                    continue
    ...
    def add_job(
        ...
        job = CronJob(
            ...
            state=CronJobState(
                next_run_at_ms=(
                    None
                    if schedule.kind == "cron" and not cron_ok
                    else _compute_next_run(schedule, now)
                ),
                last_status=("error" if schedule.kind == "cron" and not cron_ok else None),
                last_error=(
